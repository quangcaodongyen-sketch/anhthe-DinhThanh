...
const getAi = (): GoogleGenAI => {
    ...
    // 2. Nếu không có, lấy từ biến môi trường Vercel (nếu được cấu hình)
...
const parseGeminiError = (error: unknown): string => {
    ...
    if (error instanceof Error && error.message === "API_KEY_MISSING") {
        return "Chưa có API Key. Vui lòng nhấn vào biểu tượng chìa khóa vàng ở góc trên để nhập Key Gemini miễn phí.";
    }
    
    const defaultMessage = "Lỗi kết nối AI. Vui lòng kiểm tra lại API Key hoặc kết nối mạng.";
    if (error instanceof Error && error.message) {
        ...
        if (msg.includes("quota") || msg.includes("429")) {
            return "API Key này đã hết hạn mức sử dụng miễn phí trong ngày hôm nay.";
        }
...
export const analyzeImageForRestoration = async (base64ImageData: string, mimeType: string): Promise<{ prompt: string | null; error: string | null; }> => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-3-flash-preview',
            contents: { parts: [{ inlineData: { data: base64ImageData, mimeType } }, { text: "Phân tích ảnh cũ này và đề xuất prompt phục hồi nó." }] }
        });
...
export const restoreImage = async (
    ...
    try {
        ...
        throw new Error('AI không tạo được ảnh. Hãy thử yêu cầu khác.');
    } catch (error) {
...
export const changeImageBackground = async (base64ImageData: string, mimeType: string, color: 'white' | 'blue'): Promise<{ image: string | null; error: string | null; }> => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: `Thay nền ảnh sang màu ${color === 'white' ? 'trắng' : 'xanh'} trơn.` }, { inlineData: { data: base64ImageData, mimeType } }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const upscaleImage = async (base64ImageData: string, mimeType: string, factor: number): Promise<{ image: string | null; error: string | null; }> => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: `Upscale x${factor}, cực kỳ chi tiết.` }, { inlineData: { data: base64ImageData, mimeType } }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const generate360Video = async (base64ImageData: string, mimeType: string): Promise<string> => {
    ...
    let operation = await ai.models.generateVideos({
        ...
        prompt: "360 degree slow camera orbit.",
...
export const animatePortrait = async (base64ImageData: string, mimeType: string): Promise<string> => {
    ...
    let operation = await ai.models.generateVideos({
        ...
        prompt: "Subtle facial movements, natural blinking.",
        image: { imageBytes: base64ImageData, mimeType },
...
export const removeObjectFromImage = async (base64Image: string, base64Mask: string, mimeType: string) => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: "Xóa vật thể vùng được đánh dấu trắng." }, { inlineData: { data: base64Image, mimeType } }, { inlineData: { data: base64Mask, mimeType: 'image/png' } }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const applyProColor = async (base64: string, mime: string) => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: "Làm đẹp chân dung chuyên nghiệp." }, { inlineData: { data: base64, mimeType: mime } }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const recolorImage = async (base64: string, mime: string, style: string) => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: `Chỉnh màu ảnh: ${style}` }, { inlineData: { data: base64, mimeType: mime } }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const applyArtisticStyle = async (base64: string, mime: string, style: string) => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: `Áp dụng phong cách nghệ thuật: ${style}` }, { inlineData: { data: base64, mimeType: mime } }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const restoreDocument = async (base64: string, mime: string) => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: "Làm sạch văn bản, phẳng giấy, rõ chữ." }, { inlineData: { data: base64, mimeType: mime } }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const mimicImageStyle = async (subject: any, style: any) => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: "Mimic style. Preserve facial identity of second image." }, { inlineData: style }, { inlineData: subject }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const generateStyledImageFromPrompt = async (base64: string, mime: string, promptText: string) => {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: { parts: [{ text: `${promptText}. Giữ nguyên khuôn mặt.` }, { inlineData: { data: base64, mimeType: mime } }] },
            config: { responseModalities: [Modality.IMAGE] }
        });
        for (const candidate of response.candidates || []) {
            for (const part of candidate.content?.parts || []) {
...
export const changeSubjectBackground = async (subject: any, bg: any) => {
    try {
        const ai = getAi();
        const parts: any[] = [{ text: "Thay nền ảnh chuyên nghiệp." }, { inlineData: subject }];
